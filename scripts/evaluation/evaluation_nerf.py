...
GT_BASE_PATH = "/home/doan/VA_0223/eval/data/tanks_and_temples/gt"  # Added ground truth base path
...
def process_scene(scene_name, use_gpu=False):
    ...
    if os.path.exists(gt_mesh_path) and os.path.exists(pred_mesh_path):
        ...
        print(f"F-score: {metrics_3d['f_score']:.4f}")
        print(f"Precision: {metrics_3d['precision']:.4f}")
        print(f"Recall: {metrics_3d['recall']:.4f}")
    else:
